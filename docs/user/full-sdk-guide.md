...
[Agent surface](agent-surface.md) for Codex/Claude Code setup, MCP tools,
and bundled skills.
...
| Portfolio workspaces | Build, refresh, sync, render, localize, explain, and inspect connected ODPC/ODPS/ODPG portfolio workspaces from objectives, use cases, signals, and product source lanes with business document intake |
...
The root README is intentionally a short PyPI-friendly landing page. Use the
focused references below for implementation details:

- [Documentation index](../README.md): organized map of user guides, development notes, planning docs, and reports.
- [API reference](API.md): Agent API, spec helper namespaces, ODPS models, validators, serialization, and examples.
- [Agent surface](agent-surface.md): MCP server, ARWS manifest, and bundled skills for agent hosts.
- [Command guide](commands.md): what each common CLI command does, what it reads, and what it writes.
- [ODPR recipe quick start](../recipes/quick-start.md): create a project-local recipe workspace from packaged starters.
- [ODPR recipe agent usage](../recipes/agent-usage.md): safe discovery, explanation, dry-run planning, and guarded execution flow for agents.
- [ODPR RecipeCatalog](../recipes/catalog.md): metadata-only starter and project recipe discovery.
- [ODPR recipe examples](../recipes/examples.md): complete example workspace map.
- [LLM generation](generation.md): Ollama, embedded llama.cpp, or configured external LLM source-doc to ODPC fragment and ODPG graph workflow.
- [ODPR recipe workflows](recipe-workflows.md): recipe files, runner config, provider readiness, dry-run planning, guarded execution, review status, and run manifests.
- [Provider and model matrix](provider-model-matrix.md): direct hosted and local provider/model lookup.
- [Embedded llama.cpp guide](llama-cpp.md): install the optional extra, configure a GGUF model path, and run local generation without a separate LLM server.
- [NVIDIA NIM guide](nvidia-nim.md): run SDK generation through a
  local NVIDIA NIM LLM container.
- [Z.ai GLM guide](zai-glm.md): run SDK generation through the hosted
  Z.ai GLM OpenAI-compatible API.
- [LLM selection guide](llm-selection-guide.md): opinionated local and hosted model choices by SDK workflow.
- [Generation development notes](../development/generation.md): contributor-facing prompt pipeline, ODPS normalization, validation, repair, and testing guidance.
- [Portfolio development notes](../development/portfolio.md): contributor-facing portfolio workspace orchestration, renderer, localization, validation, and testing guidance.
- [Input document expansion notes](../development/sdk-input-documents-plan.md): portfolio lane document intake formats, optional Outlook support, budget gates, and warning-only source classes.
- [Development notes index](../development/README.md): contributor-facing internals notes for complex SDK surfaces.
- [Data Contract workflows](data-contracts.md): ODPS contract resolution, optional `datacontract-cli`, alignment, and reports.
- [Capability drift reports](../reports/capability-drift/README.md): dated SDK alignment reports against upstream specification tooling.
- [Tooling development model](../planning/tooling-development-model.md): human-facing explanation of how spec-level scripts mature into consolidated SDK capabilities.
- [Functional test report](../reports/functional-test-report.md): public API, CLI, and MCP functional coverage matrix.
...
[command guide](commands.md) for what each command reads, checks, and
produces.
...
generation config. See [LLM generation](generation.md#llm-setup).
...
See [Data Contract workflows](data-contracts.md) for product contract
...
[LLM generation](generation.md) for runnable provider examples.
...
See [LLM generation](generation.md) for source documents, prompts,
